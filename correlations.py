...
class Correlation:
    ...
    def apply_ROI(self):
        ...
        Si la ROI est vide, la méthode ne fait rien. Le format de la ROI doit être
        """
        if self.ROI:
    ...
    def obtain_number_of_atoms_per_cycle_in_box(self, df, box, column_name="N1"):
        """
    ...
    def counts_atoms_in_boxes_one_variable(self, df, var, box, column_name="N_1"):
        ...
        """
        # Petite bar pour faire patienter
        with alive_bar(var.n_step, title="Gathering {}".format(var.name)) as bar:
            # On parcourt les différentes valeurs de la Variable var
    ...
    def merge_dataframe_on_cycles(self, df1, df2):
        """
        Merge 2 dataframe sur les cycles. Met 0 si le df2 n'a pas de valeur à ce cycle.
        """
        df_merged = df1.merge(
